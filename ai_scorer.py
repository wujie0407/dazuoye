"""
单Bin版实时评分系统 - 支持 AI 图片显示
监控一个 Bin 中的所有设计，只对新设计评分，并显示 AI 生成的图片
"""
...
class AIEnabledRealtimeScorer:
    ...
    def calculate_score(self, design: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'score': score,
                'parameters': params,
                'ai_image_url': design.get('ai_image_url'),  # 保存 AI 图片 URL
                'success': True
            }
    ...
    def display_score(self, score_data: Dict[str, Any]):
        ...
        # 显示 AI 图片 URL
        if score_data.get('ai_image_url'):
            print(f"\n🎨 AI 效果图: {score_data['ai_image_url'][:60]}...")
    ...
    def save_score_summary(self, score_data: Dict[str, Any]):
        ...
        summary = {
            ...
            'ai_image_url': score_data.get('ai_image_url')  # 保存 AI 图片 URL
        }
    ...
    def run_continuous(self):
        ...
        print("  ✅ 自动识别新设计")
        print("  ✅ 只对每个设计评分一次")
        print("  ✅ 显示 AI 生成的效果图 URL")
        print(f"\n⏱️  检查间隔: {self.check_interval} 秒")
        print("💡 在设计器中添加新设计会自动评分并显示 AI 图片")
        print("\n按 Ctrl+C 停止\n")
        print("="*60)
        ...
        try:
            ...
            if self.score_summary:
                ...
                for summary in self.score_summary[-5:]:
                    ai_indicator = "🎨" if summary.get('ai_image_url') else "  "
                    print(f"  {ai_indicator} {summary['design_id']}: {summary['score']}/100")
...
def main():
    ...
    scorer = AIEnabledRealtimeScorer(API_KEY, check_interval=3)
